...
import retriever.retriever.utils as utils

# import scripts.retriever.tools.retriever.utils as utils
...
def store_contents(data_path, save_path, preprocess, num_workers=None):
    ...
    with tqdm(total=len(files)) as pbar:
        for pairs in tqdm(workers.imap_unordered(get_contents, files)):
            ...
            pbar.update()
    logger.info('Read %d paragraphs.' % count)
    logger.info('Committing...')
    conn.commit()
    conn.close()
    print('Saving database to', save_path)
...
if __name__ == '__main__':
    data_path = 'output/retriever/pgt_handbook_paragraph.txt'
    save_path = 'output/retriever/pgt_handbook_paragraph.db'
    store_contents(
        data_path, save_path, None, None
    )
